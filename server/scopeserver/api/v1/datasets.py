...
async def coordinates(
    ...
    if not crud.is_admin(current_user) and not crud.user_has_access_to_dataset(database, current_user, found_dataset):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="You have no access to this dataset")
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Loom file for dataset {dataset_id} could not be located",
        )
