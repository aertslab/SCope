...
class LoomFileHandler:
    def __init__(self):
        self.active_looms = {}
        self.loom_dir = dfh.DataFileHandler.get_data_dir_path_by_file_type(
            file_type="Loom"
        )

    def add_loom(
        self, partial_md5_hash: str, file_path: str, abs_file_path: str, loom_connection
    ):
        loom = Loom(
    ...
    def load_loom_file(
        self, partial_md5_hash: str, file_path: str, abs_file_path: str, mode: str = "r"
    ):
        try:
    ...
    def change_loom_mode(
        self, loom_file_path: str, mode: str = "r", partial_md5_hash: str = None
    ):
        abs_file_path = self.get_loom_absolute_file_path(loom_file_path)
        if partial_md5_hash is None:
            partial_md5_hash = LoomFileHandler.get_partial_md5_hash(
                abs_file_path, 10000
            )
        if not os.path.exists(abs_file_path):
            raise ValueError(
                "The file located at " + abs_file_path + " does not exist."
            )
        ...
        if mode == "r+":
            ...
            self.active_looms[abs_file_path] = self.get_loom(
                loom_file_path=loom_file_path, mode="r+"
            )
            logger.info(
                f"{loom_file_path} now {self.active_looms[abs_file_path].get_connection().mode}"
            )
        ...
        new_partial_md5_hash = LoomFileHandler.get_partial_md5_hash(
            abs_file_path, 10000
        )
        logger.debug(
            f"Old MD5 is: {partial_md5_hash} New MD5 is: {new_partial_md5_hash}"
        )
        ...
        if partial_md5_hash != new_partial_md5_hash:
            try:
                os.remove(
                    os.path.join(
                        os.path.dirname(abs_file_path), partial_md5_hash + ".ss_pkl"
                    )
                )
    ...
    def set_global_data(self) -> None:
        self.global_looms = [
            x
            for x in os.listdir(self.loom_dir)
            if not os.path.isdir(os.path.join(self.loom_dir, x))
        ]
    ...
    def get_loom(self, loom_file_path: str, mode: str = "r"):
        ...
        if not os.path.exists(abs_loom_file_path):
            ...
            raise ValueError(
                "The file located at " + abs_loom_file_path + " does not exist."
            )
        # To check if the given file path is given specified url!
        partial_md5_hash = LoomFileHandler.get_partial_md5_hash(
            abs_loom_file_path, 10000
        )
        ...
        logger.debug(
            f"Returning newly loaded loom file {loom_file_path}. Hash {partial_md5_hash}, object {id(loom)}"
        )
        return loom
