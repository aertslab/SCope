...
class LegacySCopeServer:
    """ Legacy SCope server. """

    def __init__(self, config, new_server=None):
        self.__new_server = new_server
    ...
    def start_data_server(self) -> None:
        ...
        self.gs_thread = threading.Thread(
            target=gs.serve,
            args=(self.run_event, self.config, self.__new_server),
        )
...
def run() -> None:
    ...
    scope_server = LegacySCopeServer(config)
    scope_server.run()
