...
function runCheckCommand(command, callback = null, sync = false, checkCommandList = []) {
    ...
    if(sync) {
        ...
        proc = exec(command, (error, stdout, stderr) => {
            ...
            if(stdout.length > 0) {
                console.log(stdout)
                if(callback === null) {
                    console.log("Callback function should be specified when executing a command asynchronously.")
                    process.exit()
                }
                callback()
            }
