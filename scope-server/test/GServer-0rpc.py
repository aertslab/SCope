...
class GServer(object):
    ...
    def get_loom_connection(self, loom_file_path):
        ...
        partial_md5_hash = GServer.get_partial_md5_hash(
    ...
    def getCellColorByFeatures(self, lfp, f1, f2, f3, lte):
        # request content
        #   - lfp   = .loom file path
        #   - e     = entries
        #   - f     = features
        ...
        feature_1_val = self.get_gene_expression(
            loom_file_path=lfp, gene_symbol=f1, log_transform=lte)
        feature_2_val = self.get_gene_expression(
            loom_file_path=lfp, gene_symbol=f2, log_transform=lte)
        feature_3_val = self.get_gene_expression(
            loom_file_path=lfp, gene_symbol=f3, log_transform=lte)
        ...
        return json.dumps(hex_vec.tolist())

    def getFeatures(self, lfp, q):
        # request content
        #   - q   = query text
        return self.get_features(lfp, q)

def main():
    print('Starting GServer on port 4242...')
    s = zerorpc.Server(GServer())
    s.bind("tcp://*:4242")
    s.run()
