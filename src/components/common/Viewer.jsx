...
export default class Viewer extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      this.getPoints(
        ...
        () => {
          ...
          if (DEBUG)
            console.log(nextProps.name, 'features active', featuresActive);
          if (featuresActive) {
            this.getFeatureColors(
              this.state.activeFeatures,
              nextProps.loomFile,
    ...
    if (
      ...
      JSON.stringify(nextProps.colors) != JSON.stringify(this.state.colors)
    ) {
...
*/
  onResize(width, height) {
    ...
    if (
      ...
      (bbox.height != 0 && (2 * VIEWER_MARGIN < dh || dh < -2 * VIEWER_MARGIN))
    ) {
      this.w = bbox.width - VIEWER_MARGIN;
      this.h = bbox.height - VIEWER_MARGIN;
      this.resizeContainer();
    ...
    if (this.state.trajectory.nodes.length > 0 && !settings.hideTrajectory) {
      ...
      for (let i = 0; i < this.state.trajectory.edges.length; i++) {
        ...
        const edgeSourceNodeScaledCoordinates = this.scalePoint(
            {
              x: this.state.trajectory.coordinates[edgeSourceNodeIndex].x,
              y: -this.state.trajectory.coordinates[edgeSourceNodeIndex].y
            },
            this.scalingFactor
          ),
          edgeTargetNodeScaledCoordinates = this.scalePoint(
            {
              x: this.state.trajectory.coordinates[edgeTargetNodeIndex].x,
              y: -this.state.trajectory.coordinates[edgeTargetNodeIndex].y
            },
            this.scalingFactor
          );
        trajectory.moveTo(
  ...
  onActiveFeaturesChange(features, featureID, customScale) {
    if (
      ...
      this.getJSONFeatures(features, 'featureType') !=
        this.getJSONFeatures(this.state.activeFeatures, 'featureType') ||
      (this.props.thresholds &&
        this.getJSONFeatures(features, 'threshold') !=
          this.getJSONFeatures(this.state.activeFeatures, 'threshold'))
    ) {
      this.setState({ loading: true });
        ...
        if (s.src != this.props.name) {
          if (s.translations[this.props.name]) {
            ...
            if (s.loomFilePath != this.props.loomFile) {
              ...
              if (DEBUG)
                console.log(this.props.name, 'translateLassoSelection', query);
        ...
        gbc.services.scope.Main.getCellColorByFeatures(
          ...
          (err, response) => {
            if (response.error !== null) {
              ...
              if (DEBUG)
                console.log(this.props.name, 'getFeatureColors', response);
