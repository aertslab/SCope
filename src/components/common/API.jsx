class API {
  constructor() {
    ...
    try {
      if (REVERSEPROXYON) {
        ...
        this.GBCConnection = new this.GBC(
          BACKEND.wsProtocol + '://' + BACKEND.host + ':' + this.WSport + '/',
          'src/proto/s.proto',
          { scope: { Main: BACKEND.host + ':' + this.RPCport } }
        ).connect();
      }
  ...
  obtainNewUUID(ip, onSuccess) {
    BackendAPI.getConnection().then((gbc) => {
      ...
      gbc.services.scope.Main.getUUID(query, (err, response) => {
        ...
        if (response != null) onSuccess(response.UUID, response.timeout);
      });
    });
  }
  getActiveLoom() {
  ...
  hasActiveCoordinatesTrajectory() {
    ...
    if (!('trajectory' in this.getActiveLoomMetaDataEmbedding())) return false;
                ...
                BackendAPI.queryLoomFiles(
                  ...
                  () => {
                    BackendAPI.getActiveFeatures().forEach((f, n) => {
                      if (
                        f.metadata['clusteringID'] == clusteringID &&
  ...
  setColabAnnotationData(feature, annotationData, orcidInfo, uuid, callback) {
    if (DEBUG)
      console.log('setColabAnnotationData', feature, annotationData, orcidInfo);
    let loomFilePath = this.getActiveLoom();
    let query = {
      loomFilePath: loomFilePath,
