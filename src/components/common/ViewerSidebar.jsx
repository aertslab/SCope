              ...
              <Grid.Row columns={3} key={i} className='selectionRow'>
                ...
                <Grid.Column>
                  <Icon
                    ...
                    style={{ opacity: lS.selected ? 1 : 0.5 }}
                    className='pointer'
                  />
                  &nbsp;
                  <Icon
              ...
              content: (
                <p>
                  {[
                    ...
                    <b>
                      {' '}
                      BETA: Some SCope functionality may be imparied until the
                      loom is reloaded
                    </b>
                  ]}
                </p>
          ...
          if (
            ...
            return (
              <Input
                ...
                action={{
                  onClick: () => {
                    this.handleAnnoUpdate(activeFeatures[i], i);
                  },
          ...
          if (
            ...
            return (
              <Grid>
                ...
                <Grid.Row>
                  <Button
                    onClick={() => this.gotoNextCluster(i, 'previous')}
                    className='change-cluster-button'>
                    {<Icon name='long arrow alternate left' />}Previous
                  </Button>
                  <CollaborativeAnnotation feature={activeFeatures[i]} id={i} />
                  <Button
                    onClick={() => this.gotoNextCluster(i, 'next')}
                    className='change-cluster-button'>
                    Next{<Icon name='long arrow alternate right' />}
                  </Button>
                </Grid.Row>
              </Grid>
            );
            ...
            let newCellTypeAnnoTableOboCell = (props) => {
              let iriLink =
                props.value.ols_iri == '' ? (
                  ...
                  <a href={props.value.ols_iri} target='_blank'>
                    {props.value.annotation_label}
                    <br />
                    {props.value.obo_id ? '(' + props.value.obo_id + ')' : ''}
                  </a>
              ...
              return (
                <React.Fragment>
                  <Popup
                    ...
                    trigger={
                      <Button
                        onClick={() => submitVote(props.value.data, 'for')}
                    ...
                    content={
                      props.value.votes_for.voters.length > 0
                        ? props.value.votes_for.voters.map((v, i) => (
                            <font color={v.voter_hash ? 'green' : 'red'}>
                              {'' + v.voter_name}&nbsp;&nbsp;
                            </font>
                          ))
                        : 'None'
                  ...
                  <Popup
                    ...
                    trigger={
                      <Button
                        onClick={() => submitVote(props.value.data, 'against')}
        ...
        if (md.genes) {
                  ...
                  if (activePage == 'regulon') {
                    ...
                    history.push(
                      '/' +
                        [
                          match.params.uuid,
                          match.params.loom ? match.params.loom : '*',
                          'gene'
                        ].join('/')
                    );
                  }
                      ...
                      gbc.services.scope.Main.getFeatures(
                        ...
                        (err, response) => {
                          BackendAPI.setActiveFeature(
                            ...
                            props.value,
                            0,
                            { description: response.featureDescription[0] }
                          );
                        }
                      );
                    },
          ...
          let markerTableHeaderName = () => {
            ...
            downloadButtonName = () => {
              if (activeFeatures[i].featureType == 'regulon')
                return (
                  'Download ' + activeFeatures[i].feature + ' regulon genes'
                );
              else if (activeFeatures[i].featureType.startsWith('Clustering'))
                ...
                <Button
                  ...
                  onClick={() => {
                    let query = {
                      ...
                      featureName: activeFeatures[i].featureType.replace(
                        /Clustering: /g,
                        ''
                      ),
                    ...
                    BackendAPI.getConnection().then(
                      (gbc) => {
                        if (DEBUG)
                          console.log('Download subset of active .loom');
          ...
          <Grid>
            <Grid.Row columns='1' centered className='viewerRow'>
              <Grid.Column className='viewerCell'>
                {activeFeatures[i] ? activeFeatures[i].featureType : ''}{' '}
                <b> {activeFeatures[i] ? activeFeatures[i].feature : ''} </b>
              </Grid.Column>
            </Grid.Row>
            {metadata}
          </Grid>
