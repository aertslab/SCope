        ...
        if (metadata) {
          return (
            <Table.Row key={j} textAlign='center'>
              ...
              <Table.Cell>
                {metadata.aucValues[0] ? metadata.aucValues[0].features[j] : ''}
              </Table.Cell>
              <Table.Cell>
                {metadata.aucValues[1] ? metadata.aucValues[1].features[j] : ''}
              </Table.Cell>
              <Table.Cell>
                {metadata.aucValues[2] ? metadata.aucValues[2].features[j] : ''}
              </Table.Cell>
              {annotation.length ? (
              ...
              {clustering != null ? (
                metadata.clusterIDs.map((c, k) => (
                  <Table.Cell key={k}>
                    {c ? selectedClustering[c.clusters[j]] : ''}
                  </Table.Cell>
                ))
              ) : (
                <Table.Cell>&nbsp;</Table.Cell>
          ...
          (cellsErr, cellsResponse) => {
            ...
            gbc.services.scope.Main.getCellMetaData(query, (err, response) => {
              ...
              this.setState({
                loading: false,
                metadata: response,
                selection: selections[this.state.selectionId],
