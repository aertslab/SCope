...
class Compare extends Component {
      ...
      if (configuration == 'one')
        return (
          <span style={{ float: 'right' }}>
            ...
            -{' '}
            <a className='pointer' onClick={this.selectNoAnotations.bind(this)}>
              none
            </a>
          </span>
        );
                  ...
                  {annotation.values.map((value, valueID) => {
                    return (
                      <Annotation
                        ...
                        value={value}
                        key={valueID}
                        isDropped={this.isDropped(annotation.name, value)}
                        onClick={this.selectAnnotation.bind(this)}
                      />
                    );
                  })}
                </Menu>
              ...
              {_.times(columns, (j) => {
                ...
                if (
                  ...
                  if (configuration == 'simple')
                    ca = crossAnnotations['both'][columns * i + j];
                  if (configuration == 'one')
                    ca = crossAnnotations['one'][columns * i + j];
                ...
                if (
                  (configuration == 'cross' || configuration == 'multi') &&
                  j == 0
                ) {
                  annotationDropContainerVertical = (
                    <AnnotationDropContainer
                ...
                if (configuration == 'multi' && i == 0) {
                  ...
                  if (
                    ...
                    multiMetadata[j].cellMetaData.embeddings.length
                  ) {
  ...
  handleRemove(viewer, name, value, orientation, position) {
    if (DEBUG)
      console.log('handleRemove', viewer, name, value, orientation, position);
  ...
  renderExpressionGraph(data) {
    ...
    Object.keys(selectedAnnotations).map((annotation, ai) => {
      ...
      state
        ...
        .append('g')
        .attr('transform', function(d) {
          return 'translate(' + (x_1(d.group) + 5 + translate) + ',0)';
        })
        .call(boxplot);
    });
  }
