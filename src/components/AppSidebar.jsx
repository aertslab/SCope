...
class AppSidebar extends Component {
  ...
  render() {
          ...
          <p>
            {[
              ...
              <Input
                ...
                onChange={(e) =>
                  this.setState({ newHierarchy_L1: e.target.value })
                }
              />,
              <br />,
              'Level 2: ',
              <Input
                ...
                onChange={(e) =>
                  this.setState({ newHierarchy_L2: e.target.value })
                }
              />,
              <br />,
              'Level 3: ',
              <Input
                ...
                onChange={(e) =>
                  this.setState({ newHierarchy_L3: e.target.value })
                }
              />,
              <br />,
              <b>
                {' '}
                BETA: Some SCope functionality may be imparied until the loom is
                reloaded
              </b>
            ]}
          </p>
    ...
    let showTransforms =
      metadata &&
      ['welcome', 'dataset', 'tutorial', 'about'].indexOf(match.params.page) ==
        -1
        ? true
        : false;
    ...
    let renderLevel = (t, l, name, canRemove) => {
          ...
          <Link
            ...
            to={
              ...
              [
                match.params.uuid,
                loomUri,
                match.params.page == 'welcome' ? 'gene' : match.params.page
              ].join('/')
            }
            onClick={() => {
            ...
            <Menu.Item
              ...
              {canRemove && (
                <Icon
                  ...
                  style={{ display: 'inline' }}
                  onClick={(e, d) =>
                    this.deleteLoomFile(file.loomFilePath, file.loomDisplayName)
                  }
                  className='pointer'
                />
              )}
              ...
              {!this.state.downloadPercentage && (
                <Icon
                  ...
                  style={{ display: 'inline' }}
                  onClick={(e, d) =>
                    this.downloadLoomFile(file.loomFilePath, file.loomSize)
                  }
                  className='pointer'
                />
              )}
      ...
      let children = Object.keys(t.children).map((level) => {
        return (
          <div key={l + '-level-' + level}>
            ...
            {!t.children[level].collapsed
              ? renderLevel(t.children[level], l + 1, null, canRemove)
              : ''}
          </div>
        );
      });
      return (
        <div key={name}>
          {name ? (
            <Menu.Header className={'level' + (l - 1)}>{name}</Menu.Header>
          ) : (
            ''
          )}
          {nodes}
      ...
      <Sidebar
        ...
        <Menu.Menu>
          ...
          {renderLevel(
            userLoomTree,
            1,
            this.props.sessionMode == 'rw' ? 'User uploaded' : 'Session Looms',
              ...
              <div>
                ...
                <Menu.Item>
                  <Checkbox
                    ...
                    onChange={this.toggleHideTrajectory.bind(this)}
                  />
                </Menu.Item>
              </div>
            )}
          ...
          <Menu.Menu>
            ...
            <Menu.Item>
              <Checkbox
                ...
                onChange={this.toggleDissociateViewers.bind(this)}
              />
            </Menu.Item>
          </Menu.Menu>
        )}
  ...
  deleteLoomFile(loomFilePath, loomDisplayName) {
    ...
    let execute = confirm(
      'Are you sure that you want to remove the file: ' + loomDisplayName + ' ?'
    );
    if (execute) {
      let query = {
        UUID: match.params.uuid,
